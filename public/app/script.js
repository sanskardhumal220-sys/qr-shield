...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function evaluateHybridDecision(mlResult, ruleResult) {
    ...
    // Base Rule Checks
    if (ruleResult.sslStatus.includes("Unencrypted")) {
        riskScore += 30;
        reasons.push("HTTP (no SSL) [+30]");
    }
    if (ruleResult.shortenerStatus.includes("Shortened")) {
        riskScore += 25;
        reasons.push("Shortened URL [+25]");
    }
    if (ruleResult.vectors.some(v => v.title.includes("Keyword"))) {
        riskScore += 20;
        reasons.push("Suspicious keywords [+20]");
    }
    if (ruleResult.contentType && ruleResult.contentType.includes("Dynamic Script")) {
        riskScore += 15;
        reasons.push("Dynamic endpoint [+15]");
    }

    // Trust & ML Modifiers
    if (isWhitelisted) {
        riskScore -= 40;
        reasons.push("Trusted domain [-40]");
    }
    if (isMlSafe) {
        riskScore -= 20;
        reasons.push("ML SAFE prediction [-20]");
    } else if (isMlMalicious) {
        riskScore += 30;
        reasons.push("ML DANGEROUS prediction [+30]");
    }
    ...
    if (gsbStatus === "Flagged") {
        riskScore = 100;
        reasons.push("Google Safe Browsing: FLAGGED [MAX RISK]");
    }

    // Clamp score
    riskScore = Math.max(0, Math.min(100, riskScore));

    // Threshold-Based Final Verdict
    ...
    // Explanability Generation
    let decisionReason = "";
    if (finalVerdict === "SAFE") {
        if (reasons.length === 0) decisionReason = "Payload appears completely clean.";
        else decisionReason = `Safe because: ${reasons.join(", ")}`;
    } else if (finalVerdict === "SUSPICIOUS") {
        decisionReason = `Suspicious due to: ${reasons.join(", ")}`;
    } else {
        decisionReason = `Dangerous due to: ${reasons.join(", ")}`;
    }

    let decisionCase = `SCORE: ${riskScore}/100`;
    ...
    return { finalVerdict, riskScore, decisionCase, decisionReason, tags };
  }
  ...
  function renderResultsDashboard(content, analysis, finalDecision, mlResult) {
    ...
    const score = finalDecision ? finalDecision.riskScore : analysis.riskScore;
